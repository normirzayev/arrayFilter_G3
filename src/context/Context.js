...
function ContextProvider({ children }) {
  ...
  let categoryBtn = ["", "smartphones", "laptops", "accessuar"];
  ...
  function sendFunc(e) {
    ...
    localStorage.setItem(
      "user",
      JSON.stringify([...userData, { ...inputValue, id: Date.now() }])
    );
    refreshLocal();
    link("/about");
  }

  // local storagedagi malumotlarni ochirish funksiyasi
  let handleDelete = (ID) => {
    localStorage.setItem(
      "user",
      JSON.stringify(userData.filter((item) => item.id !== ID))
    );
    refreshLocal();
  };
